# [Minimum Spanning Tree](https://liu.kattis.com/courses/AAPS/AAPS25/assignments/uqu3ow/problems/minspantree)

### Input

The input consists of several test cases. Each test case starts with a line with two non-negative integers, $1 <= n <= 20, 000$ and $0 <= m <= 30, 000$, separated by a single space, where $n$ is the numbers of nodes in the graph and $m$ is the number of edges. Nodes are numbered from $0$ to $n-1$. Then follow $m$ lines, each line consisting of three (space-separated) integers $u$, $v$ and $w$ indicating that there is an edge between $u$ and $v$ in the graph with weight $-20, 000 <= w <= 20, 000$. Edges are undirected.
