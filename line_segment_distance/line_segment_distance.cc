/**
 * Solves the line segment distance problem.
 *
...
/**
 * Computes the minimum distance from point p to line segment from a to b.
 *
 * First the function checks if the segment is actually just a point by seeing
 * if a and b are the same, in that case it just returns the distance from p to a.
 * Otherwise it finds the closest point on the segment using projection. The idea
 * is to project point p onto the line through a and b using the parametric form
 * where any point on that line is a + t * (b - a). The parameter t tells where
 * along the line the projection falls. We find t by computing the dot product of
 * the vector from a to p with the direction vector from a to b and dividing by
 * the squared length of the direction vector. This t could put us anywhere
 * on the extended line but we need the closest point to be on the actual semgent
 * so we limit t to be between 0 and 1. When t is 0 the closest point is a and
 * when t is 1 the closest point is b and anything in between is somewhere on the
 * segment. After limiting t we compute the closest point and return the distance
 * from p to it.
 *
 * Time Complexity: O(1)
 * Space Complexity: O(1)
